...
func NewPersistentChromeCollection(llmClient *openai.Client, collectionName, dbPath, filePath, embeddingModel string, maxChunkSize, chunkOverlap int, chunkStrategy string) *PersistentKB {
	...
	persistentKB, err := NewPersistentCollectionKB(
		...
		filePath,
		chromemDB,
		maxChunkSize, chunkOverlap, llmClient, embeddingModel, chunkStrategy)
	if err != nil {
...
func NewPersistentLocalAICollection(llmClient *openai.Client, apiURL, apiKey, collectionName, dbPath, filePath, embeddingModel string, maxChunkSize, chunkOverlap int, chunkStrategy string) *PersistentKB {
...
func NewPersistentPostgresCollection(llmClient *openai.Client, collectionName, dbPath, filePath, embeddingModel string, maxChunkSize, chunkOverlap int, databaseURL, chunkStrategy string) *PersistentKB {
